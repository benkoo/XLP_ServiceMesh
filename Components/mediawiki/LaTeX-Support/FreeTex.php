...
function returnTexImg($code, $argv) {
	$wiki_install_path = "/var/www/html";
	$density = 100;
	$quality = 100;
	...
	$cmd_cvt = sprintf("convert -quality %d -density %d ".$hash.".pdf ".$wiki_install_path."/images/teximg/".$hash.".png", $quality, $density);
	$cmd_gen_return = "";
	exec($cmd_gen, $cmd_gen_return);
	exec($cmd_cvt);
	if(file_exists($wiki_install_path."/images/teximg/".$hash.".png"))
		$url_return = '<img src="/images/teximg/'.$hash.'.png">';
	else if(file_exists($hash.".pdf") and file_exists($wiki_install_path."/images/teximg/".$hash."-0.png")) {
		#$url_return = "<h3>Error: The document has multiple pages</h3>";
		$url_return = '<img src="/images/teximg/'.$hash.'-0.png">';
		$pic_index = 1;
		while(file_exists(sprintf($wiki_install_path."/images/teximg/".$hash."-%d.png", $pic_index))) {
